...
const Navbar = () => {
  ...
  return (
    <nav  className="w-full flex py-8 md:py-14 justify-between items-center navbar px-7 md:px-20" aria-labelledby="app-menu">
      ...
      <img src={themeLogo} alt="metric logo" className="w-auto h-[48px] mr-2" />

      <ul   className="list-none sm:flex hidden justify-end items-center flex-1">
        {navLinks.map((nav, index) => (
          <li  className='px-5' key={index}  >
            <NavLink  to={nav.path} end={nav.path !== 'products' ? true : false} id={nav.id} aria-labelledby={nav.title}
                ...
                <ul   className="products-menu absolute shadow-xl bg-white rounded-xl p-4 mt-2 z-50">
                  {
                    nav.subMenu.map((menu, menuIndex) => (
                      <li    className="mb-3 last:mb-0 hover:text-primaryGreen w-full hover:cursor-pointer" key={menuIndex}>
                        <NavLink   to={menu.path} end={nav.path !== 'products' ? true : false} aria-labelledby={'sub-nav-item-' + menuIndex} className="font-poppins w-full"
