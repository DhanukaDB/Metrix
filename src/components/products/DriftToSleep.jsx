                                    ...
                                    </div>
                                    {
                                       type.amazonLink ?
                                          <div className="my-10 text-center">
                                             <a href={type.amazonLink} aria-labelledby={formatTitles(product.title) + '-order-on-amazon-' + indexTwo} title={'amazon order link for ' + product.title + ' ' + indexTwo} target="_blank"
                                          ...
                                          <div className="block md:flex md:flex-row md:space-x-10" role='region'>
                                             {
                                                type.subTypes.map((sType, indexThree) => (
                                                   <div key={indexThree} className="mb-10 md:mb-0">
                                                      ...
                                                      <div className="mt-10 mb-5 text-center text-[#000000] font-bold uppercase">
                                                         Order on Amazon as
                                                      </div>
                                                      <div className="flex flex-row space-x-3 justify-center items-center">
                                                         {
                                                            sType.pieces.map((piece, indexFour) => (
                                                               <a href={piece.url} aria-labelledby={indexThree + '-select-pair-amount-' + indexFour} title={'Link for order number of pairs from amazon ' + indexThree + '-' + indexFour} target="_blank" key={indexFour} >
